...
class RTDETRPipeline:
    """
    Orchestrates preprocessing → level estimation → RT-DETR detection
    ...
    def __init__(self, cage_type: str = "default"):
        weights = os.getenv("RTDETR_WEIGHTS", "models/rtdetr/best.pt")
        device  = os.getenv("RTDETR_DEVICE",  "cpu")
        ...
        self.detector      = RTDETRDetector(weights_path=weights, device=device)

        # Warm up so first real frame doesn't pay init cost
    ...
    def run(
        ...
        # ── Step 3: RT-DETR mouse detection ───────────────────────
    ...
    def debug_frame(self, frame: np.ndarray) -> np.ndarray:
        """
        Returns 640×640 frame annotated with ROI zones and level overlays.
        """
        frame_640 = self.preprocessor.resize(frame)
